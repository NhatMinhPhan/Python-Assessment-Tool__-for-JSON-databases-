...
def test_case_1() -> Union[bool, str]:
    ...
    # Expect circle's radius == 5
    expected = 5
    result = circle.get_radius()
    
    if expected != result:
        message = f'circle() = Circle() expects radius of {expected} units, but got {result}'
        return message
    return True
...
def test_case_2() -> Union[bool, str]:
    ...
    if expected != result:
        message = f'circle() = Circle(8) expects radius of {expected} units, but got {result}'
        return message
    return True
